...
import {
  downloadEtsyImportTemplate,
  getEtsyDetectedColumns,
  parseEtsyOrderFulfilmentCsv,
  parseEtsyOrderFulfilmentMatrix,
} from '../utils/etsyOrderFulfilmentCsvImport.js';
import { parseSpreadsheetFile } from '../utils/spreadsheetImport.js';

export default function EtsyOrderFulfilmentImportDialog({
  open,
  onClose,
  stores,
  selectedStoreId,
  ...
  const detectedColumns = parsed ? getEtsyDetectedColumns(parsed.headerMap) : [];
  ...
  return (
    <Dialog open={open} onClose={importing ? undefined : onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        <Stack spacing={2.5}>
          <Alert severity="info">
            Upload one file per Etsy store. For correct <strong>Tracking Id</strong> values, use{' '}
            <strong>.xlsx</strong> from Google Sheets (File → Download → Microsoft Excel). CSV can truncate long tracking numbers.
          </Alert>
          ...
          {stores.length === 0 && (
            <Alert severity="info">
              ...
              , then import orders here.
            </Alert>
          )}

          <Stack direction="row" spacing={1}>
            <Button size="small" startIcon={<DownloadIcon />} onClick={downloadEtsyImportTemplate}>
          ...
          >
            ...
            <Typography variant="body2" gutterBottom>
              Drag & drop CSV or Excel (.xlsx) here, or choose a file
            </Typography>
            <Button component="label" variant="contained" size="small" sx={{ mt: 1 }}>
              Choose file
              <input
                hidden
                type="file"
          ...
          {parsing && (
            <Stack spacing={1}>
              ...
              <Typography variant="caption" color="text.secondary" align="center">
                Reading file… large Excel sheets may take a few seconds.
              </Typography>
            </Stack>
          )}
