...
function cleanMessageBodyForDisplay(body = '') {
...
function ChatDialog({ open, onClose, order }) {
  ...
  async function loadMessages(showLoading = true) {
    ...
    try {
      ...
      setMessages((data || [])
        .map((msg) => ({ ...msg, body: cleanMessageBodyForDisplay(msg.body) }))
        .filter((msg) => msg.body));
    } catch (e) {
        ...
        <Box sx={{ flex: 1, p: 2, overflowY: 'auto' }}>
          {loading ? (
            ...
            <Stack spacing={2}>
              ...
              {messages.map((msg) => {
                const displaySender = getDisplaySender(msg);
                return (
                <Box
                  key={msg._id}
                  sx={{
