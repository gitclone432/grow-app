...
/**
 * Search Amazon for a template's configured keyword (or an ad-hoc keyword),
 * returning ASIN rows for review before sending them into the precheck run.
 * Feeds the "Template + Account" sourcing page (/admin/asin-sourcing).
 */
router.post(['/amazon-search-scrape', '/api/amazon-search-scrape'], requireAuth, async (req, res) => {
  try {
    const { templateId, keyword, priceMin, priceMax, region, pages } = req.body || {};
    ...
    if (templateId) {
      const template = await ListingTemplate.findById(templateId).select('sourcing name').lean();
      if (!template) {
        return res.status(404).json({ error: 'Template not found' });
      }
      const sourcing = template.sourcing || {};
      effectiveKeyword = effectiveKeyword || sourcing.searchKeyword;
      effectivePriceMin = effectivePriceMin ?? sourcing.priceMin;
      effectivePriceMax = effectivePriceMax ?? sourcing.priceMax;
      effectiveRegion = effectiveRegion || sourcing.region;
    }

    if (!effectiveKeyword || !String(effectiveKeyword).trim()) {
      return res.status(400).json({ error: 'This template has no search keyword configured, and none was provided.' });
    }

    const rows = await searchAmazonAsins({
      keyword: effectiveKeyword,
      region: ['US', 'UK', 'CA', 'AU'].includes(effectiveRegion) ? effectiveRegion : 'US',
      pages: pages || 1,
    ...
    res.json({
      ...
      priceMin: effectivePriceMin ?? null,
      priceMax: effectivePriceMax ?? null,
      count: rows.length,
      rows,
    });
  } catch (error) {
    console.error('[Amazon Search Scrape] Error:', error.message);
    res.status(error.response?.status && error.response.status < 500 ? error.response.status : 500).json({
      error: error.message || 'Amazon search scrape failed',
    });
  }
});
...
router.get('/asin-precheck-stream', requireAuthSSE, async (req, res) => {
  ...
  try {
    ...
    const { activeSkuSet, rowByAsin } = await loadActiveSkuSet(sellerId, asins);
